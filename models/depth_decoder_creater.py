...
class DepthDecoder_AdaptiveSize(tf.keras.Model):
    def __init__(self):
        super(DepthDecoder_AdaptiveSize, self).__init__()
        self.num_ch_enc = [64, 64, 128, 256, 512]
        self.num_ch_dec = [16, 32, 64, 128, 256]
        self.scales = [0,1,2,3]  # range(4)
        self.num_output_channels = 1
    ...
    def call(self, input_features, training=None, mask=None):
        ...
        for i in range(4, -1, -1):
            ...
            if i > 0:
                ...
                skip = tf.image.resize(skip, x[0].shape[1:3])
                x += [skip]
            x = tf.concat(x, ch_axis)
            x = tf.pad(x, [[0, 0], [1, 1], [1, 1], [0, 0]], mode='REFLECT')
            # x = self.convs[("upconv", i, 1)](x)
...
if __name__ == '__main__':
    ...
    decoder.summary()
    for i in range(6):
        decoder = frozen_depth_dec(dep_dec=decoder, num_frozen=i)
    a = 0
