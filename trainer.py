...
from playground.test_inverse_warp import inverse_warp_exp
import numpy as np
...
class Trainer:
    ...
    def init_app(self):
        ...
        self.models['pose_dec'] = PoseDecoder_exp(pose_num=self.opt.pose_num, has_depth=True)
    ...
    def generate_images_pred(self, inputs, outputs):
        ...
        for scale in range(self.opt.num_scales):
            ...
            for i, f_i in enumerate(self.opt.frame_idx[1:]):
                ...
                if self.opt.use_cycle_consistency and scale == 0:
                    ...
                    # warped_data = self.inverse_warp_exp(depth_tgt_s, disp_src_s0, outputs, f_i)
                ...
                # data_resamp = self.inverse_warp_exp(depth_tgt_s, src_data, outputs, f_i)
        ...
        if self.opt.use_cycle_consistency:
            ...
            outputs[('warped_multi_s', f_i, 0)].append(
                ...
                # self.inverse_warp_exp(depth_src, tgt_image, outputs, f_i)
            )
    ...
    def get_occlu_aware_mask(self, outputs):
        ...
        for f_i in self.opt.frame_idx[1:]:
            ...
            outputs[('disp_warp', f_i, 0)].append(
                ...
                # self.inverse_warp_exp(depth_src, disp_tgt, outputs, f_i)
            )
            # ---------------------------
    ...
    def inverse_warp_exp(self, depth_goal, data_to_warp, outputs, f_i):
        reverse_order = f_i < 0
        ...
        warped = inverse_warp_exp(intrinsics, rot_angles, translations, depth_goal, data_to_warp)
        return warped
